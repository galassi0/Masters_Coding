function [position, orientation, midpoints_pos, midpoints_ori, velocity_tang] = calcPose_Kuka( ...
...
% midpoints:        position and orientation (ZYX)
    ...
    % initial_ori = rad2deg(rotm2eul_c(initial_pose(1:3,1:3), 'ZYX'));
    %-----------%
    if pitch == 0
    ...
    for i = 1:N
        yaw2(i,1) = deg2rad(yaw_changes(i,1));
    ...
    % midpoint for going from first point, onwards
    for i = 1:numel(pitch_changes)-1
        midpoints(i,1) = mean(pitch_changes(i:i+1));
    ...
    for i = 1:N
        ...
        R2 = Rx(roll2(i)) * Ry(pitch2(i)) * Rz(yaw2(i));
        H2(:,:,i) = [R2 t2; 0 0 0 1];                     % H2: rotate to second orientation (no translation), assign to homogenous matrix (local frame)
        H3 = [eye(3) t3; 0 0 0 1];                        % H3: translate along local Z'' of rotated frame (becomes local frame 2)
        H_global(:,:,i) = H1 * H2(:,:,i) * H3;            % multiply (from right to left) local transpose with original orientation/position transpose (neck location) -- transpose from local to global frame
    end


    for i = 1:N-1
        pitch_midpoint(i,1) = deg2rad(pitch_mid_changes(i,1));
    end
    
    for i = 1:N-1
        % R2 = Rz(yaw2(i)) * Ry(pitch2(i)) * Rx(roll2(i));  % rotation matrix for individual movement
        R2_mid = Rx(roll2(i)) * Ry(pitch_midpoint(i)) * Rz(yaw2(i));
        H2_mid(:,:,i) = [R2_mid t2; 0 0 0 1];                     % H2: rotate to second orientation (no translation), assign to homogenous matrix (local frame)
        H3_mid = [eye(3) t3; 0 0 0 1];                        % H3: translate along local Z'' of rotated frame (becomes local frame 2)
        H_global_mid(:,:,i) = H1 * H2_mid(:,:,i) * H3_mid;            % multiply (from right to left) local transpose with original orientation/position transpose (neck location) -- transpose from local to global frame
    end
